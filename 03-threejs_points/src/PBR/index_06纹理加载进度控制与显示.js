/**********
 * 目标：通过LoadingManager管理纹理加载的进度与现实
 *      代码：32 ~ 49
***********/
 ...
 * */ 
 let event = {};
 // 图片加载进度
 event.onProgress = function(e, num, total) { 
     console.log(`正在加载 ${e} , 当前是第 ${num} 张图片，总共 ${total} 张。 加载：${ ((num / total) * 100).toFixed(2) }%`); 
}
 // 错误回调
 event.onError = function(e) { console.log('图片加载出现错误',); }
 // 设置单张纹理图加载进度
 event.onLoad = function() { console.log('图片加载完成...'); }

// 设置加载管理器(可以管理模型、纹理等加载的内容)
...
// 导入粗糙度贴图
const doorRouphnessTexture = textureLoader.load('./textures/door/roughness.jpg');
// 导入金属贴图
...
// 创建物体（是设置多顶点 100 * 100 * 100，大白话就是：网格更加密集）
const cuteGeometry = new THREE.BoxBufferGeometry(1, 1, 1, 100, 100, 100);
// 设置材质
const material = new THREE.MeshStandardMaterial({
    ...
    displacementScale: 0.1, // 突出的比例0.1  100 * 0.1 = 10;
    roughness: 1,   // 粗糙度为0，表示物体很光滑，此时可以叫做specular(镜面反射)   // 粗糙度为1，表示物体表面很粗糙，此时可以叫做漫反射(Diffuse)这里一般调整整个物体
    roughnessMap: doorRouphnessTexture, // 这里是根据粗糙度贴图来控制局部的一些粗糙度，从而达到一个反光的效果
    opacity: 1,     // 透明度1
    metalness: 1,   // 变成金属材质
    metalnessMap: doorMetalnessTexture, // 金属贴图控制局部金属反光情况
    normalMap: doorNormalTexture,   // 如果不添加法线贴图，那看到光打上后就是一片白色，门的缝隙啥的都是一体光，但有了法线贴图
